...
class Orchestrator:
    ...
    def generate_query(self, question, entities, relations, h1_threshold=9999999, question_type=None):
        ...
        valid_walks, timeout_waittime = query_builder.to_where_statement(graph, self.parser.parse_queryresult,
                                                                         ask_query=ask_query,
        ...
        if question_type == 0 and len(relations) == 1:
            ...
            graph.find_minimal_subgraph(entities, relations, double_relation=double_relation, ask_query=ask_query,
                                        sort_query=sort_query, h1_threshold=h1_threshold)
            valid_walks_new, timeout_waittime_new = query_builder.to_where_statement(graph, self.parser.parse_queryresult,
                                                                                 ask_query=ask_query,
                                                                                 count_query=count_query,
                                                                                 sort_query=sort_query)
            timeout_waittime += timeout_waittime_new
            valid_walks.extend(valid_walks_new)

        args = Struct()
        base_path = "./learning/treelstm/"
        args.expname = "lc_quad_1,epoch=5,train_loss=0.08340245485305786"
        args.mem_dim = 150
        args.hidden_dim = 50
        args.num_classes = 2
        args.input_dim = 300
        ...
        args.lr = 0.01
        args.wd = 1e-4
        args.data = os.path.join(base_path, "data/lc_quad_1/")
        # args.cuda = False
        args.cuda = True
        try:
            print("---- Start ranking valid walks")
            scores = self.rank(args, question, valid_walks)
            print("---- Valid walks ranked")
        ...
        return valid_walks, question_type, type_confidence, timeout_waittime
    ...
    def sort_query(self, linker, kb, parser, qapair, question_type_classifier, force_gold=True):
        ...
        if qapair.answerset is None or len(qapair.answerset) == 0:
            return "-Not_Applicable", [], question_type, type_confidence, precision, recall, total_timeout_waittime
        else:
            if entities is None or ontologies is None:
                recall = 0.0
                return "-Linker_failed", [], question_type, type_confidence, precision, recall, total_timeout_waittime
            ...
            for comb in combination_list:
                if len(generated_queries) == 0:
                    generated_queries, question_type, type_confidence, timeout_waittime = self.generate_query(question, comb[0], comb[1])
                    total_timeout_waittime += timeout_waittime
                    if len(generated_queries) > 0:
                        ask_query = False
                        count_query = False
            ...
            if len(generated_queries) == 0:
                recall = 0.0
                return "-without_path", [], question_type, type_confidence, precision, recall, total_timeout_waittime
                            ...
                            for j in si:
                                ...
                                if len(answerset) > 0:
                                    if answerset == qapair.answerset:
                                        ...
                                        sparql = SPARQL(
                                            kb.sparql_query(where["where"], target_var, count_query, ask_query),
                                            ds.parser.parse_sparql)
            ...
            return "correct" if correct else "-incorrect", output_where, question_type, type_confidence, precision, recall, total_timeout_waittime
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--file_type', type=str, required=True, choices=['train', 'dev', 'test'])
    args = argparser.parse_args()

    logger = logging.getLogger(__name__)
    utility.setup_logging()

    if not os.path.isfile(f"./data/LC-QUAD/linked_{args.file_type}.json"):
        with open(f"learning/treelstm/data/lc_quad_1/LCQuad_{args.file_type}.json", 'r') as f:
            test = json.load(f)
        ...
        for i in data:
            if i['id'] in test_id:
                test_data.append(i)
        with open(f'data/LC-QUAD/linked_{args.file_type}.json', 'w') as f:
            json.dump(test_data, f)

    ds = LC_Qaud_Linked(path=f"./data/LC-QUAD/linked_{args.file_type}.json")
    ds.load()
    ds.parse()
    ...
    output_file = f'lcquad_{args.file_type}_answer_output'
    linker = Earl(path=f"data/LC-QUAD/entity_lcquad_{args.file_type}.json")
    ...
    o = Orchestrator(logger, question_type_classifier, double_relation_classifier, parser,
                     question_type_classifier_path, True)

    tmp = []
    output = []
    na_list = []

    for qa_idx, qapair in enumerate(ds.qapairs):
        print(f'{qa_idx + 1} / {len(ds.qapairs)}')
        stats.inc("total")
        ...
        if qapair.answerset is None or len(qapair.answerset) == 0:
            ...
            result, where, question_type, type_confidence, precision, recall, timeout_waittime = o.sort_query(linker, ds.parser.kb,
                                                                                            ds.parser, qapair,
            ...
            output_row["timeout_waittime"] = timeout_waittime
            logger.info(result)

        logger.info(stats)
        end_time = time.time()
        output_row["elaps"] = end_time - start_time
