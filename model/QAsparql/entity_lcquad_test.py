...
def get_earl_entities(query, url='http://localhost:4999'):
    result = {}
    ...
    response = requests.post(f'{url}/processQuery',
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--file_type', type=str, required=True, choices=['dev', 'test', 'train'])
    argparser.add_argument('--earl_url', type=str, default='http://localhost:4999')
    args = argparser.parse_args()

    with open(f'learning/treelstm/data/lc_quad_1/LCQuad_{args.file_type}.json', 'r', encoding='utf-8') as f:
        data = json.load(f)

    print(len(data))
    properties = preprocess_relations('data/dbpedia/dbpedia_3Eng_property.ttl', True)
    ...
    for idx, item in enumerate(data):
        ...
        query = item['question']
        earl = get_earl_entities(query, url=args.earl_url)
    ...
    with open(f'data/LC-QUAD/entity_lcquad_{args.file_type}.json', "w") as data_file:
