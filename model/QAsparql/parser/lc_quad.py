...
# ./data/LC-QUAD/data_v8.json {"verbalized_question": "Who are the <comics characters> whose <painter> is <Bill
# Finger>?", "_id": "f0a9f1ca14764095ae089b152e0e7f12", "sparql_template_id": 301, "sparql_query": "SELECT DISTINCT
# ?uri WHERE {?uri <http://dbpedia.org/ontology/creator> <http://dbpedia.org/resource/Bill_Finger>  . ?uri
# <https://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://dbpedia.org/ontology/ComicsCharacter>}",
...
class LC_Qaud:
    ...
    def parse(self):
        for raw_row in self.raw_data:
            ...
            self.qapairs.append(
                QApair(raw_row["corrected_question"], [], sparql_query, raw_row, str(raw_row["_id"]), self.parser))
