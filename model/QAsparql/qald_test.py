...
argparser.add_argument('--file_ext', type=str, default='json')
config_args = argparser.parse_args()
...
class Orchestrator:
    def __init__(self, logger, question_classifier, double_relation_classifer, parser, filepath, auto_train=True):
        ...
        self.dep_tree_cache_file_path = f'./caches/dep_tree_cache_{config_args.qald_ver}.json'
    ...
    def rank(self, args, question, generated_queries):
        ...
        if 2 > 1:
            ...
            output_dir = f"./output/qald/{config_args.qald_ver}"
    ...
    def generate_query(self, question, entities, relations, h1_threshold=9999999, question_type=None):
        ...
        if question_type == 0 and len(relations) == 1:
            ...
            graph.find_minimal_subgraph(entities, relations, double_relation=double_relation, ask_query=ask_query,
                                        sort_query=sort_query, h1_threshold=h1_threshold)
            valid_walks_new, timeout_waittime_new = query_builder.to_where_statement(graph,
        ...
        return valid_walks, question_type, type_confidence, timeout_waittime + timeout_waittime_new
    ...
    def sort_query(self, linker, kb, parser, qapair, question_type_classifier, force_gold=True):
        ...
        if qapair.answerset is None or len(qapair.answerset) == 0:
            ...
            for comb in combination_list:
                if len(generated_queries) == 0:
                    generated_queries, question_type, type_confidence, timeout_waittime = self.generate_query(question,
                                                                                                              comb[0],
                                                                                                              comb[1])
...
if __name__ == "__main__":
    ...
    ds = Qald(path=f'./data/QALD/{config_args.qald_ver}.{config_args.file_ext}')
    ds.load()
    ds.parse()
    ...
    output_file = f'{config_args.qald_ver}_answer_output'
    linker = Earl(path=f"data/QALD/entity_{config_args.qald_ver}.json")
    ...
    for qa_idx, qapair in enumerate(ds.qapairs):
        ...
        if qapair.answerset is None or len(qapair.answerset) == 0:
            ...
            result, where, question_type, type_confidence, precision, recall, timeout_waittime = o.sort_query(linker,
                                                                                                              ds.parser.kb,
                                                                                                              ds.parser,
                                                                                                              qapair,
